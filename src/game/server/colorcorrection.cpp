...
private:
	...
	CNetworkVar(float, m_MaxWeight);	// <-- was m_flCurWeight, now m_MaxWeight
...
// DEFINE_FIELD( m_flCurWeight, FIELD_FLOAT ) not anymore – now net is m_MaxWeight
...
SendPropFloat(SENDINFO(m_MaxWeight)),	// <-- was m_flCurWeight
...
CColorCorrection::CColorCorrection() : BaseClass()
{
	...
	m_flMaxWeight = 1.0f;
	m_MaxWeight = 0.0f;		// <-- initialize (instead of m_flCurWeight.Set(0.0f))
...
void CColorCorrection::Spawn(void)
{
	...
	{
		m_bEnabled = false;
		m_MaxWeight = 0.0f;		// <-- was m_flCurWeight.Set(0.0f)
	}
	else
	{
		m_bEnabled = true;
		m_MaxWeight = 1.0f;		// <-- was m_flCurWeight.Set(1.0f)
	}
...
void CColorCorrection::FadeIn(void)
{
	...
	m_flStartFadeInWeight = m_MaxWeight;		// <-- было m_flCurWeight
...
void CColorCorrection::FadeOut(void)
{
	...
	m_flStartFadeOutWeight = m_MaxWeight;		// <-- was m_flCurWeight
...
void CColorCorrection::FadeInThink(void)
{
	...
	if (m_flFadeInDuration <= 0 ||  // not set to fade in
		m_MaxWeight >= m_flMaxWeight ||  // already past max weight   <-- было m_flCurWeight
	...
	m_MaxWeight = Lerp(flFadeRatio, m_flStartFadeInWeight, m_flMaxWeight);	// <-- was m_flCurWeight
...
void CColorCorrection::FadeOutThink(void)
{
	...
	if (m_flFadeOutDuration <= 0 || // not set to fade out
		m_MaxWeight <= 0.0f || // already faded out   <-- было m_flCurWeight
		m_bEnabled || // fade in/out mutex
		m_flMaxWeight == 0.0f || // min==max
	...
	m_MaxWeight = Lerp(1.0f - flFadeRatio, 0.0f, m_flStartFadeOutWeight);	// <-- was m_flCurWeight
...
void CColorCorrection::InputEnable(inputdata_t& inputdata)
{
	...
	{
		m_MaxWeight = m_flMaxWeight;		// <-- was m_flCurWeight = m_flMaxWeight
	}
...
void CColorCorrection::InputDisable(inputdata_t& inputdata)
{
	...
	{
		m_MaxWeight = 0.0f;			// <-- was m_flCurWeight = 0.0f
	}
