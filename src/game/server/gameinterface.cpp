	...
	for (int i = 0; i < pTable->GetNumProps(); ++i)
	{
		...
		if (pProp->GetType() == DPT_DataTable)
		{
			...
			if (pInner)
			{
				...
				filesystem->Write(buf, Q_strlen(buf), hFile);
				WriteSendTable(hFile, pInner, level + 1);
			}
		}
		else
		{
			const char* typeName = GetDTTypeName(pProp->GetType());
			char buf[256];
...
// dump_server_props instead of sm_dump_netprops (basically they're same)
CON_COMMAND(dump_server_props, "Dump all server SendTables to server_props_dump.txt (SourceMod format)")
{
	FileHandle_t hFile = filesystem->Open("server_props_dump.txt", "w", "DEFAULT_WRITE_PATH");
	if (hFile == FILESYSTEM_INVALID_HANDLE)
	{
		Warning("Failed to create server_props_dump.txt\n");
		return;
	}
	...
	for (ServerClass* pClass = g_pServerClassHead; pClass; pClass = pClass->m_pNext)
	{
		...
		const char* tableName = pClass->m_pTable->GetName();
		char header[256];
		Q_snprintf(header, sizeof(header), "%s (type %s)\n", className, tableName);
		filesystem->Write(header, Q_strlen(header), hFile);

		WriteSendTable(hFile, pClass->m_pTable, 1);
	}

	filesystem->Close(hFile);
	Msg("Server props dumped to server_props_dump.txt (SourceMod format)\n");
}